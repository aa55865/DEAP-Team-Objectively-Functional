...
for i in range(0,num):
    if i==0:
        cav1= (ts, tb)
        cav2= tuple(map(operator.add, cav1, (((tw-tt-tb)/2)*m.tan(theta), (tw-tt-tb)/2)))
        cav3= (ts, tw-tt)
        cavlist=[cav1,cav2,cav3]
        cav_set.append(cavlist)
    if i==1 or i==4 or i==7 or i==10 or i==13 or i==16 or i==19:
        cav4= tuple(map(operator.add, cav_set[i-1][2], (ti, 0)))
        cav6= tuple(map(operator.add, cav_set[i-1][0], ((tw-tt-tb)*m.tan(theta), (tw-tt-tb))))
        cav5= ((cav_set[i-1][0][0]+cav6[0]+ti)/2, tb+(-cav_set[i-1][0][0]+(cav_set[i-1][0][0]+cav6[0]+ti)/2)/m.tan(theta))
        cavlist=[cav4,cav5,cav6]
        cav_set.append(cavlist)
    if i==2 or i==5 or i==8 or i==11 or i==14 or i==17 or i==20:
        cav8= tuple(map(operator.add, cav_set[i-2][0], (ti, 0)))
        cav7= (cav_set[i-1][1][0], tb+(cav_set[i-1][1][0]-cav8[0])/m.tan(theta))
        cav9= (cav_set[i-1][2][0], cav8[1])
        cavlist=[cav7,cav8,cav9]
        cav_set.append(cavlist)
    if i==3 or i==6 or i==9 or i==12 or i==15 or i==18:
        cav10= tuple(map(operator.add, cav_set[i-2][2], (ti, 0)))
        cav11= tuple(map(operator.add, cav_set[i-3][1], (ti, 0)))
        cav12= tuple(map(operator.add, cav_set[i-1][2], (ti, 0)))
        cav13= tuple(map(operator.add, cav12, (((tw-tt-tb)/2)*m.tan(theta), (tw-tt-tb)/2)))
        cavlist=[cav12,cav13,cav10,cav11]
        cav_set.append(cavlist)
    if i==21:
        cav1= tuple(map(operator.add, cav_set[i-2][2], (ti, 0)))
        cav2= tuple(map(operator.add, cav_set[i-3][1], (ti, 0)))
        cav3= tuple(map(operator.add, cav_set[i-1][2], (ti, 0)))
        cavlist=[cav1,cav2,cav3]
        cav_set.append(cavlist)
        tr_corner= tuple(map(operator.add, cav1, (ts, tt)))
        br_corner= tuple(map(operator.add, cav3, (ts, -tb)))
...
for i in range(0,num):
	if i==3 or i==6 or i==9 or i==12 or i==15 or i==18:
		...
		mod.sketches['__profile__'].Line(point1=cav_set[i][2], point2=cav_set[i][3])
		mod.sketches['__profile__'].Line(point1=cav_set[i][3], point2=cav_set[i][0])
	else:
		mod.sketches['__profile__'].Line(point1=cav_set[i][0], point2=cav_set[i][1])
		mod.sketches['__profile__'].Line(point1=cav_set[i][1], point2=cav_set[i][2])
...
for i in range(0,num):
	if i==3 or i==6 or i==9 or i==12 or i==15 or i==18:
		...
		cavmid3= tuple(n.divide(tuple(n.add(cav_set[i][2], cav_set[i][3])), (2, 2)))+(0.0,)
		cavmid4= tuple(n.divide(tuple(n.add(cav_set[i][3], cav_set[i][0])), (2, 2)))+(0.0,)
		mod.rootAssembly.Surface(name='cav'+str(i+1), side1Edges=mod.rootAssembly.instances['Part-1-1'].edges.findAt((cavmid1,), (cavmid2,), (cavmid3,), (cavmid4,)))
	else:
		cavmid1= tuple(n.divide(tuple(n.add(cav_set[i][0], cav_set[i][1])), (2, 2)))+(0.0,)
		cavmid2= tuple(n.divide(tuple(n.add(cav_set[i][1], cav_set[i][2])), (2, 2)))+(0.0,)
...
mod.sketches['__profile__'].Line(point1=tuple(map(operator.add, cav_set[9][2], (0, tt/2))), point2=tuple(map(operator.add, cav_set[12][2], (0, tt/2))))
mod.sketches['__profile__'].Line(point1=tuple(map(operator.add, cav_set[9][2], (0, 3*tt/2))), point2=tuple(map(operator.add, cav_set[12][2], (0, 3*tt/2))))
mod.sketches['__profile__'].Line(point1=tuple(map(operator.add, cav_set[9][2], (0, tt/2))), point2=tuple(map(operator.add, cav_set[9][2], (0, 3*tt/2))))
mod.sketches['__profile__'].Line(point1=tuple(map(operator.add, cav_set[12][2], (0, tt/2))), point2=tuple(map(operator.add, cav_set[12][2], (0, 3*tt/2))))
...
for i in range(0,num):
	mod.CavityRadiation(createStepName='Step-1', name='cav_'+str(i+1), surfaceEmissivities=('cement', ), surfaces=(mod.rootAssembly.surfaces['cav'+str(i+1)], ))
